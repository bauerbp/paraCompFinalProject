...
void testcuFFT(){
  ...
  cufftReal *h_signal = (cufftComplex *)malloc(sizeof(cufftComplex) * SIGNAL_SIZE);
...
1.44858683588};
  ...
  cufftComplex *d_data;
  cufftReal *d_signal;
  cudaMalloc((void**)&d_data, sizeof(cufftComplex)*(NX/2+1)*SIGNAL_SIZE);
  cudaMalloc((void**)&d_signal, sizeof(cufftComplex)*NX);
  ...
  if(cufftPlan1d(&plan, NX, CUFFT_R2C, SIGNAL_SIZE) != CUFFT_SUCCESS){
  ...
  cudaMemcpy(h_data,d_data,sizeof(cufftComplex)*SIGNAL_SIZE * (NX/2+1),cudaMemcpyDeviceToHost);
